...
*/
function findURLs(path, matchMethod) {
    ...
    return function (item) { 
        ...
        return  nmbrOfPartsPath == nmbrOfParts && regExp.test(path);
    }
}
...
Vatican.prototype.handleOptionsRequest = function (path) {
    ...
    if(["*", "/*"].indexOf(path) == -1 ) {
        validMethods = validMethods.filter(findURLs(path, false))
    } 
...
Vatican.prototype.findMethod = function( url, method ) {
    ...
    var match = _.find(this.paths, findURLs(path, true) );
    return match;
};
