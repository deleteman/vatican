...
*/
function findURLs(path, methodToMatch) {
    ...
    return function (item) { 
        ...
        if(methodToMatch) {
            boolExpression = boolExpression && item.method.toLowerCase() == methodToMatch;
        }
        return  boolExpression;
    }
}

Vatican.prototype.handleOptionsRequest = function (path) {

    //should also allow for url = *
    let validMethods = _(this.paths);
    if(["*", "/*"].indexOf(path) == -1 ) {
        validMethods = validMethods.filter(findURLs(path, null))
    } 
...
Vatican.prototype.findMethod = function( url, method ) {
    ...
    if(method.toUpperCase() == 'OPTIONS') {
        let validMethods = this.handleOptionsRequest(path)
        return new OptionsResponse(validMethods);
    }

    var match = _.find(this.paths, findURLs(path, method) );
    return match;
};
