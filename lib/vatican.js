...
function Vatican(options) {
    ...
    options || (options = {});
    var context = options.context = this.getContext(options.context);
    var config = this.getConfig(context);

    this.options = _.defaults(options, config, DEFAULT_CONFIG);
    this.checkOptions();

    this.requestParser = require(path.resolve(context, requestParserPath));
...
Vatican.prototype.getConfig( context ) {
    var confPath = context + "/vatican-conf.json";
    try {
...
Vatican.prototype.getContext(context) {
    if( ! context ) {
        context = process.cwd();
    } else { 
...
Vatican.prototype.checkOptions = function() {
    ...
    if( !this.options.handlers ) {
        ...
        this.options.handlers = path.resolve(this.options.context, this.options.handlers);
    }
...
Vatican.prototype.requestHandler = function (req, res) { 
    ...
    if( !methodFound ) {
        ...
        try {
            ...
            var hdlr = this.loadHandler(path.resolve(this.options.context, methodFound.handlerPath));
