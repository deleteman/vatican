...
(function(version) {
    if( version[1] < 11 || (version[1] == 11 && version[2] < 2) ) {
        path.isAbsolute = function(pth) {
            return path.resolve(pth) == pth;
        }
    }
  }
...
var DEFAULT_CONFIG = {
    ...
    requestParser:  __dirname + '/defaultRequestParser',
}

function Vatican(context, options) {
    this.dbconn = null;

    if(arguments.length < 2 && typeof context != 'string') {
        options = context;
        context = undefined;
    }
    ...
    this.options = _.defaults(options ? options : this.getConfig(), DEFAULT_CONFIG);
    this.checkOptions();

    this.requestParser = require(path.resolve(this.context, this.options.requestParser));
    this.handlers = {};
    this.parseHandlers();
    this.paths = [];
    this.server = null;
...
Vatican.prototype.getConfig = function() {
    var confPath = this.context + "/vatican-conf.json";
    try {
        return require( confPath );
    } catch(ex) {
        logger.error("Error loading config file '" + confPath + "': " + ex);
        throw new Error( "Error loading config file '" + confPath );
    }
}

Vatican.prototype.getContext = function(context) {
    if( ! context ) {
        context = process.cwd();
    } else { 
        if( !path.isAbsolute( context ) ) {
            logger.error("Context specified is not an absolute path, throwing error!");
            throw new Error("Context specified specified is not an absolute path");
        }
        ...
        try {
            ...
            if( e.code == -20 ) {
                logger.error("Context specified can not be treated as directory, throwing error!");
                throw new Error("Context specified can not be treated as directory");
            } 
...
Vatican.prototype.checkOptions = function() {
    ...
    if( !this.options.handlers ) {
        ...
        this.options.handlers = path.resolve(this.context, this.options.handlers);
    }
};
...
Vatican.prototype.requestHandler = function (req, res) { 
    ...
    if( !methodFound ) {
        ...
        try {
            ...
            var hdlr = this.loadHandler(path.resolve(this.context, methodFound.handlerPath));
