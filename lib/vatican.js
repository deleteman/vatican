...
Vatican.prototype.preprocess = function(fn) {
    this.preprocessors.add(fn)
    this.totalPreprocessors = this.preprocessors.getTotal();
}

Vatican.prototype.postprocess = function(fn) {
...
Vatican.prototype.loadHandler = function(path) {
    if(this.handlers[path]) return this.handlers[path];
    ...
    this.handlers[path] = new m.exports;
    ...
    if( !methodFound ) {
        ...
        try {
            ...
            this.parseRequest(request, methodFound.url, req, function(newRequest) {
                ...
                self.preprocessors.add(function(req, resp, next) {
                    hdlr[methodFound.action](req, resp, next);
                })
                self.preprocessors.runChain(newRequest, res);
            });
        } catch (ex) {
